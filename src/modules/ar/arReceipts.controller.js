...
const arReceiptsService = require('./arReceipts.service');
    ...
    try {
        ...
        const [allocations] = await pool.query(`
            SELECT ara.*, ai.invoice_number, ai.invoice_date, ai.total as invoice_total,
                (SELECT COALESCE(SUM(ra.allocated_amount), 0) FROM ar_receipt_allocations ra WHERE ra.invoice_id = ai.id) as invoice_received_amount,
                (ai.total - COALESCE((SELECT SUM(ra.allocated_amount) FROM ar_receipt_allocations ra WHERE ra.invoice_id = ai.id), 0)) as invoice_outstanding
    ...
    try {
        ...
        const [rows] = await pool.query(`
            SELECT ai.id, ai.invoice_number, ai.invoice_date, ai.due_date, ai.total,
                COALESCE(SUM(ra.allocated_amount), 0) as received_amount,
                (ai.total - COALESCE(SUM(ra.allocated_amount), 0)) as outstanding_amount
            FROM ar_invoices ai
