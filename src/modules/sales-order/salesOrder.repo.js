...
export const getSalesOrderItems = async (conn, { salesOrderId, clientId }) => {
    const [rows] = await conn.query(
        `SELECT soi.*, p.product_name, u.acronyms as uom_name
...
export const replaceSalesOrderItems = async (conn, { salesOrderId, clientId, items }) => {
    ...
    const values = items.map(i => [
        ...
        i.uom_id,
        i.unit_price, i.discount_type || 'PERCENTAGE', i.discount_rate || 0, i.discount_amount || 0, i.tax_rate, i.line_subtotal, i.line_tax, i.line_total
    ]);

    await conn.query(
        `INSERT INTO sales_order_items 
    (client_id, sales_order_id, product_id, description, quantity, ordered_quantity, dispatched_quantity, uom_id, unit_price, discount_type, discount_rate, discount_amount, tax_rate, line_subtotal, line_tax, line_total)
    VALUES ?`,
        [values]
    );
};
...
export const listSalesOrders = async (conn, { clientId, page, pageSize, search, status_id, company_id, customer_id, date_from, date_to, edit_request_status }) => {
    ...
    const sql = `
    SELECT so.*, v.display_name as customer_name, comp.name as company_name, 
           s.name as status_name, s.bg_colour as color_code, s.colour as status_text_color,
