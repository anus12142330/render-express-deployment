...
const generateOrderNo = async (conn, { clientId, companyId, orderDate }) => {
    const prefixRaw = await fetchCompanyPrefix(conn, companyId);
    const prefix = (prefixRaw ? String(prefixRaw).trim() : '') + 'SO';

    // Check if prefix already contains SO or similar, if not, user might expect one?
    // Existing logic was simple substitution. Keeping it simple. 

    const date = orderDate ? new Date(orderDate) : new Date();
    const yy = Number(String(date.getFullYear()).slice(-2));
    const mm = Number(String(date.getMonth() + 1).padStart(2, '0'));

    // Base format: PREFIX-YY-MM-
    const pattern = `${prefix}-${String(yy).padStart(2, '0')}-${String(mm).padStart(2, '0')}-`;

    // Find the latest order matching this pattern
    // We strictly look for orders starting with this pattern
    const [rows] = await conn.query(
        `SELECT order_no 
          ...
          ORDER BY id DESC 
          LIMIT 1`,
        [clientId, companyId, `${pattern}%`]
    );

    let nextSeq = 1;
    if (rows.length && rows[0].order_no) {
        // Assume format is ...-XXX
        const parts = rows[0].order_no.split('-');
...
export const updateDraftHeader = async ({ clientId, userId, id, payload }) =>
    withTx(async (conn) => {
        ...
        await updateSalesOrderHeader(conn, {
            ...
            updated_by: userId,
            terms_conditions: payload.terms_conditions !== undefined ? payload.terms_conditions : header.terms_conditions,
...
export const previewNextSequence = async ({ clientId, companyId, orderDate }) =>
    withTx(async (conn) => {
        return await generateOrderNo(conn, { clientId, companyId, orderDate });
    });
